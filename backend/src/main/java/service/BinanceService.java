...
public class BinanceService {
    ...
    @Transactional
    public ServiceResponseDTO<BinanceTokenDTO> deleteCredentials(String userSub) {
        return tokenRepo.findByUserSub(userSub)
                .map(token -> {
                    tokenRepo.deleteByUserSub(userSub);
                    BinanceTokenDTO dto = mapper.entityToDto(token);
                    return ServiceResponseDirector.successOk(dto, "Credentials deleted");
                })
                .orElseGet(() -> ServiceResponseDirector.errorNotFound(
                        "No Binance credentials stored for user: " + userSub
                ));
